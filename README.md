...
```mermaid
graph TD
    subgraph "1. Data Pipeline (Task 1-4)"
        A[Raw Documents: PDF, DOCX, HTML] -->|Task 3: MarkItDown| B(Standardized Markdown)
        B -->|Task 4: Chunking| C(Text Chunks)
        C -->|Embedding Model| D[(ChromaDB Vector Store)]
        C -->|Tokenization| E[(BM25 Lexical Store)]
    end

    subgraph "2. Retrieval Pipeline (Task 5-9)"
        Q[User Query] --> S1(Task 5: Semantic Search)
        Q --> S2(Task 6: Lexical Search)
        D -.-> S1
        E -.-> S2
        S1 --> M(Merge Results)
        S2 --> M
        M --> R(Task 7: Reranking)
        R --> C1{Top Score > Threshold?}
        C1 -- Yes --> FC[Final Context Chunks]
        C1 -- No --> F(Task 8: PageIndex Fallback)
        F --> FC
    end

    subgraph "3. Generation (Task 10)"
        FC -->|Task 10: Reorder chunks| P(Context Formatting)
        P -->|Inject to Prompt| LLM((LLM))
        LLM --> Out[Answer with Citations]
    end
  
    Q -.-> S3[Chatbot UI / Streamlit]
    Out -.-> S3
```

---

### Phân Công Công Việc (Phương Án B: Nhóm 5 Thành Viên - Chuyên Sâu Retrieval)

| Vai trò                                                  | Thành viên            | MSSV        | Nhiệm vụ                                                                                       | Trạng thái |
| :-------------------------------------------------------- | :---------------------- | :---------- | :----------------------------------------------------------------------------------------------- | :----------- |
| **Role 1 (Team Leader & RAG Architect)**            | Nguyễn Phương Đông | 2A202601474 | Quản lý chung, ghép code pipeline chính (`supervisor.py` & Task 9)                         | Hoàn thành |
| **Role 2 (Data & Dense Search Dev)**                | Nguyễn Quý Dũng      | 2A202601200 | Task 1–3 (Data) + Task 4 (ChromaDB) + Task 5 (Semantic Search & HyDE)                           | Hoàn thành |
| **Role 3 (Sparse Search & Advanced Reranking Dev)** | Ngô Minh Phước       | 2A202601576 | Task 6 (BM25/TF-IDF) + Task 7 (RRF Reranking) + Task 8 (PageIndex Fallback)                      | Hoàn thành |
| **Role 4 (Frontend & Chatbot Developer)**           | Trần Thị Kiều Trang  | 2A202601498 | Xây dựng Streamlit Chatbot`app.py` + Task 10 (Generation có Citation)                       | Hoàn thành |
| **Role 5 (Evaluation & QA Engineer)**               | Vũ Nhật Minh          | 2A202601950 | Bộ câu hỏi`golden_dataset.json` + Đánh giá RAGAS & báo cáo so sánh A/B `results.md` | Hoàn thành |
